...
class ApiPosInterfaceTest extends TestCase
{
    ...
    {
        $this->json('GET', '/api/pos/999?api_token='.$this->user->api_token)->seeStatusCode(404)->seeJson([ 'message' => 'Record not found' ]);
    }
    ...
    public function can_make_a_booking_with_items()
    {
        ...
        $this->json('POST', '/api/pos/'.$account->id.'?api_token='.$this->user->api_token, ['items' => $itemData])
            ->seeStatusCode(200)
            ->seeJson(
                [
